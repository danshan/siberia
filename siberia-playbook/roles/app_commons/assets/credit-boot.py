...
# Version      2.0.1
# Time         2012.11.20
# Configure    credit-boot.conf
# Control spring-boot to start, stop, restart, kill, status, info, cleanwork, backup, dump, pid, debug

__author__  = 'JiangZQ'
__version__ = '2.0.1'
__build__   = '2012.11.20'
...
DEFAULT_CONFIG_FILE = '%s/credit-boot.conf.default' % BASE_PATH
CONFIG_FILE = '%s/credit-boot.conf' % BASE_PATH
...
class App(Output):
    ...
    def execute(self, action, output=sys.stdout):
        ...
        cmd = '%s %s -classpath %s -Dapp.name=%s -Dspring.application.name=%s -Djava.io.tmpdir=%s   -Dserver.port=%s -Djava.security.egd=file:/dev/./urandom -Dserver.undertow.accesslog.dir=%s org.springframework.boot.loader.JarLauncher %s' % \
              (self.cfg.appname, java_opts, self.cfg.classpath, self.cfg.appname, self.cfg.appname, self.cfg.spring_boot_temp,self.cfg.http_port,self.cfg.log_path, action)
        cmd = shlex.split(cmd)
...
def main():
    ...
    parser.add_argument('appname', metavar='AppName', help='AppName defind in credit-boot.conf or all')
